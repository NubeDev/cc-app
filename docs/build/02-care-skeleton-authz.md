...
- [x] Milestone 01 closed (booting host).
- [x] `lb-ext-sdk` tag pinned; extension build/publish loop proven once by hand
      (rubix-ai `docs/extensions/README.md` is the reference loop).

## Work items

- [x] Extension skeleton: manifest, `build.sh`, publish-to-node flow; a `care.ping` verb
      to prove the loop; caps declared per the master scope's persona sets.
- [x] **`authz/` module** — the two-call API every verb will use:
...
- [x] **Era-2 seam stubbed:** the delegation point to lb `authz.check_scoped` /
...
- [x] **The cross-family matrix harness:** seeds the canonical fixture — Sam(Leo+Mia),
...
- [x] The grep fence in CI: `guardianship` read outside `authz/` fails the build.
