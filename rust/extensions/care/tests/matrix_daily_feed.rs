...
async fn guardian_list_sees_only_her_childs_entries() {
    ...
    assert!(
        !out.contains(MIA),
        "MUST NOT leak Mia's entries across families: {out}"
    );
    assert!(
        !out.contains(MIA_MEDIA),
        "MUST NOT leak Mia's photo media id: {out}"
    );
}
...
async fn guardian_day_allows_her_child_denies_the_other() {
    ...
    let leo = day::run(&cp, &ana, &day_input(LEO))
        .await
        .expect("Ana reaches Leo");
    assert!(
        leo.contains(LEO_MEDIA),
        "Ana's Leo rollup carries Leo's photo"
    );
    ...
    assert!(
        !err.contains(MIA_MEDIA),
        "THE MEDIA-URL LEAK: Mia's photo id must not leak: {err}"
    );
...
async fn stranger_feed_watch_is_denied() {
    ...
    let res = watch::run(
        &cp,
        &stranger,
        &serde_json::json!({"child_id": LEO}).to_string(),
    )
    .await;
    assert!(
        res.is_err(),
        "a stranger's subscribe must be denied (never handed a subject)"
    );
...
async fn incident_push_targets_both_languages_with_resolvable_keys() {
    ...
    assert!(
        d.is_push() && d.must_deliver,
        "an incident is a must-deliver push"
    );
    assert_eq!(
        d.recipients.len(),
        2,
        "both language recipients targeted from one send"
    );
    ...
    assert!(
        en_title.contains("Leo") && es_title.contains("Leo"),
        "child interpolated both langs"
    );
    assert_ne!(
        en_title, es_title,
        "en and es incident titles differ (real localization)"
    );
    ...
    assert_ne!(
        en_body, d.body_key,
        "en body resolved to words, not the key"
    );
    assert_ne!(
        es_body, d.body_key,
        "es body resolved to words, not the key"
    );
}
